...
class App extends Component {
  ...
  deleteItem = id => {
    ...
    const newList = latestList.filter(password => password.id !== id)
  ...
  render() {
    ...
    const newList = latestList.filter(eachpassword =>
      eachpassword.website.toLowerCase().includes(searchInput.toLowerCase()),
    )
    if (newList.length === 0) isTrue = false
    else isTrue = true
    return (
